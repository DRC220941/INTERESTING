...
# ===== FIXTURE POUR RÉINITIALISER LA MÉMOIRE AVANT CHAQUE TEST =====
import pytest

@pytest.fixture(autouse=True)
def reset_memory():
    """Réinitialise la mémoire de travail avant chaque test"""
    working_mem.reset()
    yield
    # Optionnel : nettoyage après le test (si nécessaire)
    working_mem.reset()
...
def test_predict_empty():
    # La mémoire est déjà réinitialisée par la fixture
    response = client.post("/predict", json={"values": []})
    assert response.status_code == 200
    assert "Données insuffisantes" in response.json()["warnings"]  # <-- Accent ajouté
