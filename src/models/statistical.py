...
class StatisticalModel:
    """ModŠle bas‚ sur des statistiques simples (moyenne, variance, tendances)"""
    ...
    def predict(self) -> Tuple[List[float], List[float]]:
        """Retourne 3 pr‚dictions + confiances"""
        ...
        # Pr‚dictions : moyenne ñ ‚cart-type
        pred_low = max(0.1, mean - std)
        pred_mid = mean
        pred_high = mean + std

        # Confiances (bas‚es sur la stabilit‚ des donn‚es)
        ...
        return [
            ...
            round(pred_mid, 2),
            round(pred_high, 2)
        ], [confidence, confidence * 1.1, confidence * 0.9]